...
function useAICoach(): UseAICoachReturn {
  ...
  const buildContext = useCallback(() => ({
    ...
    activities: activities.map(a => ({ id: a.id, name: a.name, emoji: a.emoji ?? null })),
  }), [activities])
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    const userMsg: AIMessage = { id: genId(), role: 'user', content: text.trim(), timestamp: Date.now() }
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: genId(), role: 'assistant', content, timestamp: Date.now() }])
      setStreaming('')
      const userCount = nextMessages.filter(m => m.role === 'user').length
      if (userCount >= 4 && planState !== 'draft_ready' && planState !== 'generating') {
  ...
  const generatePlan = useCallback(async () => {
    ...
    try {
      ...
      const draftTasks: DraftTask[] = sorted.map(t => ({
        ...t, reminderEnabled: t.reminderRecommended, aiPlanId: planId, done: false,
      }))
      const dates = draftTasks.map(t => t.date).sort()
      setDraftPlan({
        id: planId, createdAt: Date.now(),
        goal: plan.goal, activitySuggestion: plan.activitySuggestion,
        phases: plan.phases, milestones: plan.milestones, tasks: draftTasks,
        dateRange: { start: dates[0] ?? todayStr(), end: dates[dates.length - 1] ?? todayStr() },
  ...
  const addTasksToCalendar = useCallback(async () => {
    ...
    try {
      ...
      if (!resolvedActId) {
        const existing = activities.find(a => a.name.toLowerCase() === sugg.suggestedName.toLowerCase())
        if (existing) {
      ...
      const grouped = draftPlan.tasks.reduce<Record<string, DraftTask[]>>((acc, t) => {
        ;(acc[t.date] ??= []).push(t)
        return acc
      }, {})
      for (const [dateKey, tasks] of Object.entries(grouped)) {
        updateDay(dateKey, prev => {
          const newTasks = tasks.map(dt => ({
            id: 'ai-' + genId(),
            text: dt.title, done: false, journal: dt.notes ?? '',
            timerStart: null, timerEnd: null,
            actId: resolvedActId,
            milestone: dt.priority === 'high',
            sessions: dt.startTime && dt.endTime
              ? [{ id: 'ais-' + genId(), startTs: timeToTs(dateKey, dt.startTime), endTs: timeToTs(dateKey, dt.endTime), note: dt.notes ?? '', tags: [] }]
              : [],
      ...
      setPlanState('saved')
      setToast(`~Tasks added to calendar!\n${draftPlan.tasks.length} tasks from your AI plan`)
      setTimeout(() => { setPlanState('idle'); setDraftPlan(null); setMessages([]) }, 3000)
    } catch (err: unknown) {
  ...
  const updateDraftTask = useCallback((clientId: string, patch: Partial<DraftTask>) => {
    setDraftPlan(prev => prev ? { ...prev, tasks: prev.tasks.map(t => t.clientId === clientId ? { ...t, ...patch } : t) } : prev)
  }, [])
  ...
  const resetConversation = useCallback(() => {
    ...
    setMessages([]); setDraftPlan(null); setPlanState('idle')
    setStreaming(''); setIsStreaming(false); setErrorMsg(null)
  ...
  const startVoice = useCallback(async () => {
    ...
    try {
      ...
      mediaRecRef.current = rec; rec.start(); setVoiceState('listening')
  ...
  const stopVoice = useCallback(async (): Promise<string | null> => {
    ...
    return new Promise(resolve => {
      rec.onstop = async () => {
        rec.stream.getTracks().forEach(t => t.stop()); mediaRecRef.current = null
        const blob = new Blob(audioChunksRef.current, { type: 'audio/webm' })
        audioChunksRef.current = []
        try {
          const fd = new FormData(); fd.append('audio', blob)
          const res = await fetch('/api/ai-coach/transcribe', { method: 'POST', body: fd })
          if (!res.ok) throw new Error('Transcription failed')
          const { text } = await res.json()
          setVoiceState('idle'); resolve(text ?? null)
        } catch { setVoiceState('idle'); setToast('~Transcription failed\nCould not convert speech.'); resolve(null) }
      }
      rec.stop()
    })
  }, [voiceState, setToast])
  ...
  const cancelVoice = useCallback(() => {
    const rec = mediaRecRef.current
    if (rec) { rec.stream.getTracks().forEach(t => t.stop()); mediaRecRef.current = null }
    audioChunksRef.current = []; setVoiceState('idle')
  }, [])

  return {
    messages, streamingContent, isStreaming, planState, draftPlan, voiceState, errorMsg,
    sendMessage, generatePlan, addTasksToCalendar,
    updateDraftTask, removeDraftTask, resetConversation,
...
function WaveformDisplay({
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Central bloom
      const bloom = ctx.createRadialGradient(W * 0.5, cY, 0, W * 0.5, cY, W * 0.46)
      ...
      for (let i = 0; i < N; i++) {
        ...
        const barH = Math.max(2, H * 0.80 * env * anim)
        const y    = cY - barH * 0.5
        ...
        grd.addColorStop(0,   `hsla(${hue + 18},${sat}%,${lit + 14}%,${alph * 0.85})`)
        grd.addColorStop(0.38,`hsla(${hue},     ${sat}%,${lit}%,     ${alph})`)
        grd.addColorStop(0.62,`hsla(${hue},     ${sat}%,${lit}%,     ${alph})`)
        grd.addColorStop(1,   `hsla(${hue + 18},${sat}%,${lit + 14}%,${alph * 0.85})`)
  ...
  return (
    <div style={{ position: 'relative' }}>
      <canvas
        ref={canvasRef}
        style={{ display: 'block', width: '100%', height: 86, borderRadius: 10 }}
      />
      <div style={{
...
function ConversationArea({ messages, streamingContent, isStreaming }: {
  messages: AIMessage[]
  streamingContent: string
  isStreaming: boolean
}) {
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 12px 8px', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 9 }}>
          <div style={{
            ...
            fontSize: 16, flexShrink: 0, boxShadow: '0 2px 8px rgba(109,40,217,0.4)',
          }}>🤖</div>
          <div style={{
            flex: 1,
            ...
            <p style={{ fontSize: 13.5, fontWeight: 700, color: '#1a1033', lineHeight: 1.3, marginBottom: 5 }}>Hi Craig! 👋</p>
            ...
            <p style={{ fontSize: 12.5, color: '#7c3aed', marginTop: 7, fontWeight: 600 }}>
              What goal do you want to focus on?
            </p>
            <p style={{ fontSize: 10.5, color: '#a099b8', marginTop: 8, textAlign: 'right' }}>{timeStr}</p>
          </div>
        </div>
      </div>
    )
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
      {messages.map(msg => (
        <div key={msg.id} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start', gap: 8, alignItems: 'flex-start' }}>
          {msg.role === 'assistant' && (
            <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'linear-gradient(135deg,#6d28d9,#9333ea)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0, marginTop: 2 }}>🤖</div>
          )}
          <div style={{
            maxWidth: '78%', padding: '9px 13px',
            borderRadius: msg.role === 'user' ? '16px 16px 4px 16px' : '16px 16px 16px 4px',
            background: msg.role === 'user' ? 'linear-gradient(135deg,#5b21b6,#7c3aed)' : 'rgba(255,255,255,0.07)',
            border: msg.role === 'assistant' ? '0.5px solid rgba(255,255,255,0.1)' : 'none',
      ...
      {isStreaming && (
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
          <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'linear-gradient(135deg,#6d28d9,#9333ea)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0 }}>🤖</div>
          <div style={{ maxWidth: '78%', padding: '9px 13px', borderRadius: '16px 16px 16px 4px', background: 'rgba(255,255,255,0.07)', border: '0.5px solid rgba(255,255,255,0.1)', fontSize: 14, lineHeight: 1.55, color: 'var(--xp-txt)', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
            {streamingContent || (
              <span style={{ display: 'flex', gap: 4 }}>
                {[0,1,2].map(i => <span key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#a78bfa', display: 'inline-block', animation: `xp-blink 1s ${i * 200}ms ease-in-out infinite` }} />)}
              </span>
            )}
            {streamingContent && <span style={{ display: 'inline-block', width: 2, height: 14, background: '#a78bfa', marginLeft: 2, animation: 'xp-blink 0.8s ease-in-out infinite', verticalAlign: 'text-bottom' }} />}
          </div>
        </div>
      )}
      <div ref={bottomRef} />
...
function AICoachCard({ coach, onEnterFocusMode: _onEnterFocusMode, isActive, onActivate }: AICoachCardProps) {
  const { isDark } = useApp()
  ...
  const canGenerate  = userMsgCount >= 3 && (planState === 'ready_to_generate' || planState === 'conversation')
  const isGenerating = planState === 'generating'
  const isSaved      = planState === 'saved'
  const inputDisabled = isStreaming || isGenerating || planState === 'saving' || isSaved
  ...
  async function handleMic() {
    if (voiceState === 'listening') {
      ...
      if (t) { setText(prev => prev ? prev + ' ' + t : t); taRef.current?.focus() }
  ...
  const cardStyle = isActive ? {
    ...
    border: isDark ? '0.5px solid rgba(124,58,237,0.28)' : '1px solid rgba(124,58,237,0.2)',
    boxShadow: isDark ? '0 4px 20px rgba(0,0,0,0.45)' : '0 2px 16px rgba(124,58,237,0.07)',
  }
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: 'background-color 190ms ease-out, border-color 190ms ease-out, box-shadow 190ms ease-out',
        ...cardStyle,
      }}
    >
      {/* ── Waveform (dark, top of card, no card header) ── */}
      <div style={{ padding: '12px 12px 8px', flexShrink: 0, background: isDark ? 'rgba(8,4,20,0.98)' : '#ffffff' }}>
      ...
      <div style={{
        ...
        display: 'flex', flexDirection: 'column', minHeight: 0,
        background: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(250,248,255,0.8)',
      }}>
        <ConversationArea messages={coach.messages} streamingContent={coach.streamingContent} isStreaming={isStreaming} />
      </div>
      ...
      {messages.length === 0 && !isStreaming && !inputDisabled && (
        <div style={{ padding: '0 12px 8px', flexShrink: 0 }}>
          <div style={{
            borderRadius: 14,
            border: isDark ? '1px solid rgba(124,58,237,0.18)' : '1px solid rgba(229,224,255,0.9)',
            background: isDark ? 'rgba(124,58,237,0.05)' : 'white',
            padding: '10px 12px',
            boxShadow: isDark ? 'none' : '0 1px 4px rgba(124,58,237,0.06)',
          }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: isDark ? 'rgba(196,168,255,0.7)' : '#6d28d9', marginBottom: 8, letterSpacing: '0.04em' }}>
              ✨ Suggested Goals
            </p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {[
                ...
                <button
                  key={g.label}
                  onClick={e => { e.stopPropagation(); setText(g.label); taRef.current?.focus() }}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 5, padding: '5px 11px',
                    borderRadius: 20, cursor: 'pointer', fontSize: 12, fontWeight: 500,
                    color: isDark ? 'rgba(255,255,255,0.82)' : '#3d3456',
                    background: isDark ? 'rgba(124,58,237,0.10)' : 'rgba(243,240,255,0.9)',
                    border: isDark ? '0.5px solid rgba(124,58,237,0.28)' : '1px solid rgba(209,200,255,0.8)',
              ...
              <button
                onClick={e => { e.stopPropagation(); taRef.current?.focus() }}
                style={{
                  display: 'flex', alignItems: 'center', gap: 5, padding: '5px 11px',
                  borderRadius: 20, cursor: 'pointer', fontSize: 12, fontWeight: 500,
                  color: isDark ? 'rgba(255,255,255,0.38)' : '#a099b8',
                  background: 'transparent',
                  border: isDark ? '0.5px dashed rgba(255,255,255,0.14)' : '1px dashed rgba(180,170,220,0.6)',
                  transition: 'background 140ms ease',
                }}
              >
                + Custom goal
      ...
      {/* ── Error banner ── */}
      {coach.errorMsg && (
        <div style={{ margin: '0 14px 6px', padding: '7px 12px', borderRadius: 10, background: 'rgba(239,68,68,0.1)', border: '0.5px solid rgba(239,68,68,0.3)', fontSize: 12, color: '#fca5a5' }}>
          {coach.errorMsg}
        </div>
      )}
...
function PlaceholderTaskRow({ idx }: { idx: number }) {
  const { isDark } = useApp()
  const alpha = 1 - idx * 0.10
  return (
    <div
      aria-hidden="true"
      style={{
        borderRadius: 10,
        border: isDark ? '0.5px solid rgba(255,255,255,0.06)' : '1px solid rgba(229,224,255,0.85)',
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 10px 4px' }}>
        <span style={{ fontSize: 11, color: isDark ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.15)', flexShrink: 0, cursor: 'grab' }}>⠿</span>
        <span style={{ fontSize: 11, color: isDark ? 'rgba(255,255,255,0.22)' : '#7c6fa0', fontWeight: 600, minWidth: 36, flexShrink: 0 }}>Task {idx + 1}</span>
        <div style={{
        ...
        <div style={{
          ...
          background: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(220,215,240,0.45)',
          display: 'flex', alignItems: 'center', paddingLeft: 8,
          overflow: 'hidden',
        }}>
          <span style={{ fontSize: 11.5, color: isDark ? 'rgba(255,255,255,0.22)' : '#b0a8c8', fontStyle: 'italic' }}>
            Tasks by AI Coach
          </span>
        </div>
        {/* Activity pill */}
        <div style={{
          ...
          borderRadius: 8,
          border: isDark ? '0.5px solid rgba(255,255,255,0.08)' : '1px solid rgba(209,200,255,0.7)',
          background: isDark ? 'rgba(124,58,237,0.06)' : 'rgba(243,240,255,0.7)',
          flexShrink: 0,
        }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: isDark ? 'rgba(167,139,250,0.3)' : 'rgba(124,58,237,0.3)' }} />
          <span style={{ fontSize: 10, color: isDark ? 'rgba(167,139,250,0.35)' : '#a99dc8', fontWeight: 500 }}>Activity</span>
        </div>
        <span style={{ fontSize: 13, color: isDark ? 'rgba(255,255,255,0.10)' : 'rgba(0,0,0,0.12)' }}>···</span>
      </div>
      {/* Bottom row */}
      <div style={{
        display: 'flex', alignItems: 'center', gap: 7, padding: '3px 10px 8px',
        borderTop: isDark ? '0.5px solid rgba(255,255,255,0.04)' : '0.5px solid rgba(229,224,255,0.6)',
      }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: isDark ? 'rgba(255,255,255,0.16)' : '#c4bedd', minWidth: 38 }}>0h 0m</span>
        <div style={{ width: 22, height: 22, borderRadius: 5, background: 'rgba(22,163,74,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(22,163,74,0.28)', fontSize: 9 }}>▶</div>
        <div style={{ width: 22, height: 22, borderRadius: 5, background: 'rgba(239,68,68,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(239,68,68,0.28)', fontSize: 9 }}>■</div>
        <span style={{ fontSize: 11.5, color: isDark ? 'rgba(167,139,250,0.22)' : '#c4bedd', fontWeight: 500 }}>00:00 AM – 00:00 PM</span>
        <div style={{ marginLeft: 'auto', width: 20, height: 20, borderRadius: 5, background: isDark ? 'rgba(124,58,237,0.08)' : 'rgba(243,240,255,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: isDark ? 'rgba(167,139,250,0.25)' : 'rgba(124,58,237,0.3)' }}>▶</div>
      </div>
    </div>
  )
}

function TaskCard({ task, idx, color, onUpdate, onRemove }: {
  task: DraftTask
  idx: number
  color: string
  ...
  return (
    <div style={{
      borderRadius: 12,
      border: isDark ? '0.5px solid rgba(255,255,255,0.08)' : '0.5px solid rgba(0,0,0,0.09)',
      background: task.done
        ? isDark ? 'rgba(22,163,74,0.05)' : 'rgba(22,163,74,0.04)'
        : isDark ? 'rgba(255,255,255,0.04)' : 'rgba(248,248,252,0.9)',
      overflow: 'hidden',
    }}>
      {/* Top row: number + checkbox + title + activity + menu */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px 6px' }}>
        {/* Drag handle */}
        <span style={{ color: isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.2)', fontSize: 11, flexShrink: 0, cursor: 'grab' }}>≡</span>
        {/* Task number */}
        <span style={{ fontSize: 11, color: isDark ? 'rgba(255,255,255,0.35)' : 'rgba(0,0,0,0.35)', fontWeight: 600, minWidth: 18, flexShrink: 0 }}>
          {idx + 1}
        </span>
        {/* Checkbox */}
        <button
        ...
        >
          {task.done && <span style={{ fontSize: 9, color: 'white', fontWeight: 900 }}>✓</span>}
        </button>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {editing ? (
            <input
              ...
              style={{
                width: '100%', background: 'rgba(124,58,237,0.08)', border: '0.5px solid rgba(124,58,237,0.4)',
                borderRadius: 6, padding: '3px 8px', fontSize: 13, color: 'var(--xp-txt)', outline: 'none',
              }}
            />
          ) : (
            <span
        ...
        <div style={{
          display: 'flex', alignItems: 'center', gap: 4, padding: '2px 8px',
          borderRadius: 8, border: `0.5px solid ${color}55`,
          background: isDark ? `${color}18` : `${color}12`,
          flexShrink: 0, maxWidth: 110,
        }}>
          <div style={{ width: 7, height: 7, borderRadius: '50%', background: color, flexShrink: 0 }} />
          <span style={{ fontSize: 11, color: isDark ? 'rgba(255,255,255,0.7)' : 'rgba(0,0,0,0.65)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 500 }}>
            {task.activityName}
          </span>
        </div>
        ...
        <button
          onClick={onRemove}
          style={{ width: 20, height: 20, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', background: 'none', cursor: 'pointer', color: isDark ? 'rgba(255,255,255,0.25)' : 'rgba(0,0,0,0.3)', fontSize: 14 }}
        >
          ···
        </button>
      </div>
      ...
      <div style={{
        ...
        borderTop: isDark ? '0.5px solid rgba(255,255,255,0.05)' : '0.5px solid rgba(0,0,0,0.06)',
      }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: isDark ? 'rgba(255,255,255,0.45)' : 'rgba(0,0,0,0.4)', minWidth: 42 }}>
          {fmtMs(totalMs)}
        </span>
        {/* Start/stop buttons */}
        <button style={{ width: 22, height: 22, borderRadius: 5, background: 'rgba(22,163,74,0.12)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#16a34a', fontSize: 10 }}>▶</button>
        <button style={{ width: 22, height: 22, borderRadius: 5, background: 'rgba(239,68,68,0.1)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9 }}>🟥</button>
        {/* Time range */}
        {task.startTime && task.endTime ? (
          <span style={{ fontSize: 12, color: isDark ? 'rgba(167,139,250,0.8)' : '#7c3aed', fontWeight: 500 }}>
            {fmtTime(task.startTime)} – {fmtTime(task.endTime)}
          </span>
        ) : (
          <span style={{ fontSize: 12, color: 'var(--xp-txt3)' }}>{fmtDate(task.date)}</span>
        )}
        {task.priority === 'high' && (
          <span style={{ fontSize: 9, fontWeight: 700, color: '#f59e0b', letterSpacing: '0.06em', background: 'rgba(245,158,11,0.12)', padding: '1px 5px', borderRadius: 3, marginLeft: 'auto' }}>HIGH</span>
        )}
        {task.reminderEnabled && <span style={{ fontSize: 11, marginLeft: 'auto' }}>🔔</span>}
        <div style={{ marginLeft: 'auto', width: 20, height: 20, borderRadius: 5, background: 'rgba(124,58,237,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: '#9333ea' }}>▶</div>
      </div>
    </div>
  )
}
...
function TaskManagerCard({ coach, onCollapse: _onCollapse, isActive = false, onActivate }: TaskManagerCardProps) {
  const { isDark } = useApp()
  ...
  const tasksByDate = useMemo(() => {
    ...
    for (const t of draftPlan.tasks) { (groups[t.date] ??= []).push(t) }
  ...
  const tmCardStyle = isActive ? {
    ...
    background: isDark ? 'rgba(8,4,22,0.98)' : '#ffffff',
    border: isDark ? '0.5px solid rgba(124,58,237,0.22)' : '1px solid rgba(0,0,0,0.08)',
    boxShadow: isDark ? '0 4px 20px rgba(0,0,0,0.45)' : '0 2px 16px rgba(0,0,0,0.06)',
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '10px 12px 8px', borderBottom: isDark ? '0.5px solid rgba(255,255,255,0.06)' : '0.5px solid rgba(229,224,255,0.9)', flexShrink: 0 }}>
        {/* Row 1: date-range pill + action buttons */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          <div style={{
            border: isDark ? '1px solid rgba(124,58,237,0.38)' : '1.5px solid #7c3aed',
            borderRadius: 10, padding: '5px 12px', fontSize: 13,
            color: isDark ? 'rgba(255,255,255,0.9)' : '#3d3456', fontWeight: 600,
          ...
          <button style={{
            ...
            background: '#7c3aed', color: 'white', fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer',
          ...
          <button style={{
            display: 'flex', alignItems: 'center', gap: 5, padding: '5px 11px', borderRadius: 8,
            background: '#16a34a', color: 'white', fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer',
            whiteSpace: 'nowrap', flexShrink: 0,
          }}>
            + Add Task
          </button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ fontSize: 12, color: isDark ? 'rgba(255,255,255,0.55)' : '#6b6080' }}>
            Total tasks: <strong style={{ color: isDark ? 'white' : '#1a1033' }}>
              {draftPlan ? `${draftPlan.tasks.length}/5` : '0/5'}
            </strong>
            <span style={{ marginLeft: 3, fontSize: 10, opacity: 0.5 }}>∨</span>
          </span>
          <span style={{ fontSize: 12, color: isDark ? 'rgba(255,255,255,0.55)' : '#6b6080', marginLeft: 10 }}>
            Total Focus Time Today: <strong style={{ color: isDark ? 'white' : '#1a1033' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 12px' }}>
        {!draftPlan ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            {Array.from({ length: N_PLACEHOLDERS }, (_, i) => <PlaceholderTaskRow key={i} idx={i} />)}
          </div>
        ) : (
          <>
            {draftPlan.warnings.length > 0 && (
              <div style={{ marginBottom: 10, padding: '8px 12px', borderRadius: 10, background: 'rgba(245,158,11,0.08)', border: '0.5px solid rgba(245,158,11,0.25)' }}>
            ...
            {tasksByDate.map(([date, tasks]) => (
              <div key={date} style={{ marginBottom: 12 }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--xp-txt3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6, paddingLeft: 2 }}>
                  {fmtDate(date)}
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                  {tasks.map((t, i) => (
      ...
      <div style={{ padding: '10px 12px 12px', borderTop: isDark ? '0.5px solid rgba(255,255,255,0.06)' : '0.5px solid rgba(229,224,255,0.9)', flexShrink: 0 }}>
        {isSaved ? (
          <div style={{ width: '100%', padding: '12px 0', borderRadius: 12, fontSize: 14, fontWeight: 700, textAlign: 'center', background: 'rgba(22,163,74,0.12)', border: '0.5px solid rgba(22,163,74,0.4)', color: '#86efac', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ✓ Tasks Added to Calendar
          </div>
        ) : isSaving ? (
          <button disabled style={{ width: '100%', padding: '12px 0', borderRadius: 12, fontSize: 14, fontWeight: 700, background: 'linear-gradient(135deg,#5b21b6,#7c3aed)', color: 'rgba(255,255,255,0.75)', border: 'none', cursor: 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: 0.85 }}>
            <span style={{ display: 'inline-block', width: 14, height: 14, borderRadius: '50%', border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', animation: 'xp-spin 0.7s linear infinite' }} />
            Adding Tasks…
          </button>
        ) : (
          <button
...
function WorkspaceNavBar({ active, onChange }: {
  active: WorkspaceTab
  ...
  return (
    <div style={{ display: 'flex', borderTop: '0.5px solid rgba(255,255,255,0.1)', background: 'var(--xp-hdr)', flexShrink: 0 }}>
      {tabs.map(tab => {
        ...
        return (
          <button
            ...
            style={{
              ...
              background: 'transparent', borderRight: 'none', borderBottom: 'none', borderLeft: 'none',
...
export function AICoachPage({ onClose }: AICoachPageProps) {
  ...
  const [activePanel,   setActivePanel]   = useState<'coach' | 'tasks' | 'calendar'>('coach')
  ...
  const calBorder = calIsActive
    ...
    : isDark ? '0.5px solid rgba(124,58,237,0.22)' : '1px solid rgba(124,58,237,0.18)'
  const calShadow = calIsActive
    ...
    : isDark ? '0 -4px 20px rgba(0,0,0,0.25)' : '0 -2px 10px rgba(124,58,237,0.06)'
  const calBg = calIsActive
    ? (isDark ? 'rgba(10,5,26,0.97)' : '#fdfbff')
    : (isDark ? 'rgba(8,4,22,0.95)' : '#ffffff')
      ...
      return (
        ...
        >
          <div style={{ flex: 1, overflowY: 'auto' }}>
            <CalendarSection onDayDoubleClick={() => {}} onMonthZoom={() => {}} activeMonth={null} />
          </div>
        </div>
      )
    }
  ...
  const ThreePanelBody = (
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', padding: '12px 12px 0', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <AICoachCard
          coach={coach}
          onEnterFocusMode={() => { setPanelFocusMode(true); setActiveTab('coach') }}
      ...
      <div
        ...
        style={{
          flex: 1, minHeight: 140, marginTop: 12,
  ...
  const SinglePanelBody = (
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ flex: 1, minHeight: 0, overflow: 'hidden', padding: '12px 14px 0', display: 'flex', flexDirection: 'column' }}>
        {renderSinglePanel()}
      </div>
      <WorkspaceNavBar
        active={activeTab}
        onChange={tab => { setActiveTab(tab); setActivePanel(tab) }}
      />
    </div>
  )
  ...
  return (
    <div
      ...
      style={{ background: isDark ? 'rgba(0,0,0,0.82)' : 'rgba(0,0,0,0.55)', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}
      onClick={onClose}
    >
      {/* Ambient glows */}
      {isDark && (
        <div className="fixed inset-0 pointer-events-none overflow-hidden" style={{ zIndex: -1 }}>
          <div style={{ position: 'absolute', top: '-8%', right: '-4%', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle, rgba(124,58,237,0.07) 0%, transparent 65%)' }} />
          <div style={{ position: 'absolute', bottom: 0, left: '-6%', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle, rgba(8,145,178,0.04) 0%, transparent 65%)' }} />
        </div>
      )}
